...
class Amasty_Promo_Model_Observer
{
    ...
    * */
        ...
        if (Mage::getStoreConfig('ampromo/messages/show_stock_warning')) {
            ...
            foreach ($rulesData as $rule) {
                ...
                if (!in_array($rule['simple_action'], $this->_arrayWithProductSet)) {
                    $actionsAndConditions[] = Mage::helper('ampromo')->unserialize($rule['actions_serialized']);
                    $actionsAndConditions[] = Mage::helper('ampromo')->unserialize($rule['conditions_serialized']);
                }
            }
    ...
    {
        $skus = $actionsAndConditions ? $this->_recGetArrayWithSkus($actionsAndConditions, 'value') : array();
