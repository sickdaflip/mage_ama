...
abstract class Amasty_Promo_Model_Rule_Action_ActionAbstract
{
    ...
    {
        if (($rule->getSimpleAction() !== 'ampromo_spent' && isset($this->_isHandled[$rule->getId()]))
            || isset($this->_isHandled[$rule->getId()][$item->getId()])) {
            return false;
        }
        ...
        if ($rule->getSimpleAction() === 'ampromo_spent') {
            $this->_isHandled[$rule->getId()] = array();
            $this->_isHandled[$rule->getId()][$item->getId()] = true;
        }
        ...
        foreach ($promoSku as $sku) {
            ...
            if ($rule->getSimpleAction() === 'ampromo_spent' && !$this->hasFieldChanged($item, 'qty')) {
     ...
     * @return bool
     */
    private function hasFieldChanged($item, $field)
    {
        return $item->getData($field) != $item->getOrigData($field);
    }
}