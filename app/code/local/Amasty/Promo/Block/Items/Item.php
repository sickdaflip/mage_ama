...
class Amasty_Promo_Block_Items_Item extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function getPriceHtml($product, $displayMinimalPrice = false, $idSuffix = '')
    {
        ...
        if ($product->getAmpromoShowOrigPrice()) {
            ...
            if ($specialPrice) {
                $originalPrice = $store->roundPrice($store->convertPrice($product->getPrice() ?: 0));
                $specialPrice = $store->roundPrice($store->convertPrice($specialPrice));

                $html = str_replace($originalPrice, $specialPrice, $html);
            }
        }
